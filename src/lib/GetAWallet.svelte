<script lang="ts">
  import { darkMode } from './stores/DarkModeStore';
</script>

<div class="flex flex-col gap-4">
  <div>
    <div class="sc-kLgxMn BASWU" data-projection-id="463">
      <svg
        aria-hidden="true"
        width="298"
        height="188"
        viewBox="0 0 298 188"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        ><path
          d="M1 55.2757L21.6438 46.0285C55.5896 30.8228 94.4104 30.8228 128.356 46.0286L169.644 64.5229C203.59 79.7287 242.41 79.7286 276.356 64.5229L297 55.2757M1 44.2118L21.6438 34.9646C55.5896 19.7589 94.4104 19.7589 128.356 34.9646L169.644 53.459C203.59 68.6647 242.41 68.6647 276.356 53.459L297 44.2118M1 33.1477L21.6438 23.9005C55.5896 8.69479 94.4104 8.69479 128.356 23.9005L169.644 42.3949C203.59 57.6006 242.41 57.6006 276.356 42.3949L297 33.1477M1 22.1477L21.6438 12.9005C55.5896 -2.30521 94.4104 -2.30521 128.356 12.9005L169.644 31.3949C203.59 46.6006 242.41 46.6006 276.356 31.3949L297 22.1477M1 66.3398L21.6438 57.0926C55.5896 41.8869 94.4104 41.8869 128.356 57.0926L169.644 75.587C203.59 90.7927 242.41 90.7927 276.356 75.587L297 66.3398M1 77.404L21.6438 68.1568C55.5896 52.9511 94.4104 52.9511 128.356 68.1569L169.644 86.6512C203.59 101.857 242.41 101.857 276.356 86.6512L297 77.404M1 88.4681L21.6438 79.2209C55.5896 64.0152 94.4104 64.0152 128.356 79.2209L169.644 97.7153C203.59 112.921 242.41 112.921 276.356 97.7153L297 88.4681M1 121.66L21.6438 112.413C55.5896 97.2075 94.4104 97.2075 128.356 112.413L169.644 130.908C203.59 146.113 242.41 146.113 276.356 130.908L297 121.66M1 110.596L21.6438 101.349C55.5896 86.1433 94.4104 86.1433 128.356 101.349L169.644 119.843C203.59 135.049 242.41 135.049 276.356 119.843L297 110.596M1 99.5321L21.6438 90.2849C55.5896 75.0792 94.4104 75.0792 128.356 90.2849L169.644 108.779C203.59 123.985 242.41 123.985 276.356 108.779L297 99.5321M1 132.724L21.6438 123.477C55.5896 108.271 94.4104 108.271 128.356 123.477L169.644 141.971C203.59 157.177 242.41 157.177 276.356 141.971L297 132.724M1 143.788L21.6438 134.541C55.5896 119.336 94.4104 119.336 128.356 134.541L169.644 153.036C203.59 168.241 242.41 168.241 276.356 153.036L297 143.788M1 154.853L21.6438 145.605C55.5896 130.4 94.4104 130.4 128.356 145.605L169.644 164.1C203.59 179.305 242.41 179.305 276.356 164.1L297 154.853M1 165.853L21.6438 156.605C55.5896 141.4 94.4104 141.4 128.356 156.605L169.644 175.1C203.59 190.305 242.41 190.305 276.356 175.1L297 165.853"
          stroke="url(#paint0_linear_1094_2077)"
          stroke-opacity="0.9"
          stroke-linecap="round"
          stroke-linejoin="round"
        /><defs
          ><linearGradient
            id="paint0_linear_1094_2077"
            x1="1"
            y1="112.587"
            x2="297.034"
            y2="79.6111"
            gradientUnits="userSpaceOnUse"
          >
            <stop stop-color="#E8F17D" />
            <stop stop-color="#A8ECDE" offset="0.239583" />
            <stop stop-color="#7AA1F2" offset="0.515625" />
            <stop stop-color="#DEA1E8" offset="0.739583" />
            <stop stop-color="#F46D98" offset="1" />
          </linearGradient>
          {#if !$darkMode}
            <radialGradient
              id="whiteToTransparent"
              cx="0.5"
              cy="0.5"
              r="0.5"
              gradientUnits="objectBoundingBox"
              class="s-JaucsxaUr--p"
            >
              <stop offset="0.7" stop-color="transparent" stop-opacity="0" />
              <stop offset="0.8" stop-color="transparent" stop-opacity="0" />
              <stop offset="0.9" stop-color="white" stop-opacity="0" />
              <stop offset="1" stop-color="white" stop-opacity="1" />
            </radialGradient>
          {:else}
            <radialGradient
              id="whiteToTransparent"
              cx="0.5"
              cy="0.5"
              r="0.5"
              gradientUnits="objectBoundingBox"
              class="s-JaucsxaUr--p"
            >
              <stop offset="0.7" stop-color="transparent" stop-opacity="0" />
              <stop offset="0.8" stop-color="transparent" stop-opacity="0" />
              <stop offset="0.9" stop-color="#2B2B2B" stop-opacity="0" />
              <stop offset="1" stop-color="#2B2B2B" stop-opacity="1" />
            </radialGradient>
          {/if}
        </defs>
        <rect width="100%" height="100%" fill="url(#whiteToTransparent)" />
      </svg>
    </div>
  </div>
  <div class="mb-4 flex flex-col gap-2">
    <p class="title">Start Exploring Web3</p>
    <p class="subtitle">
      Your wallet is the gateway to all things Ethereum, the magical technology that makes it
      possible to explore web3.
    </p>
  </div>

  <button
    on:click={() => {
      window.open('https://ethereum.org/en/wallets/find-wallet/', '_blank');
    }}
    class="text-base font-semibold rounded-xl border border-transparent h-11 px-3 dark:text-white dark:bg-dark-button text-light-text bg-light-button hover:bg-opacity-[60%] dark:hover:bg-opacity-[95%] cursor-pointer transition-border-color duration-200 focus:outline-none flex items-center justify-center gap-2"
  >
    Chose Your First Wallet
    <svg width="13" height="12" viewBox="0 0 13 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      <line
        stroke="currentColor"
        x1="1"
        y1="6"
        x2="12"
        y2="6"
        stroke-width="2"
        stroke-linecap="round"
        class="on-hover"
      />
      <path
        stroke="currentColor"
        d="M7.51431 1.5L11.757 5.74264M7.5 10.4858L11.7426 6.24314"
        stroke-width="2"
        stroke-linecap="round"
        class="point"
      />
    </svg>
  </button>
</div>

<style>
  .title {
    line-height: 20px;
    font-size: 17px;
    font-weight: 600;
  }
  .subtitle {
    font-size: 16px;
    font-weight: 400;
    line-height: 21px;
    color: #999;
  }
  .point {
    transition: all 75ms ease 0s;
    transform: translateX(-3px);
  }
  .on-hover {
    opacity: 0;
  }
  button:hover .on-hover {
    opacity: 1;
  }
  button:hover .point {
    transform: translateX(0px);
  }
</style>
